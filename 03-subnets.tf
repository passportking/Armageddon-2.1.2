...
# resource "aws_subnet" "tokyo-syslog-private-subnet-1a"{                
#     vpc_id = aws_vpc.tokyo-syslog-vpc.id                     
#     cidr_block = var.subnet_cidr_block_tokyo-syslog-private1a             
#     availability_zone = var.avail_zone_tokyo-syslog-private1a      
#     provider    = aws.tokyo-syslog
#     tags = {                                           
#         Name: "${var.env_prefix_tokyo-syslog}-subnet-1a"     
#         Service: "Armageddon Phase 1"
#         Owner = "MATTRESS AVENGERS"         
#     }
# }
...
resource "aws_db_subnet_group" "tokyo_aurora_database_sg" {
  name       = "tokyo_aurora_database_sg"
  #vpc_id = aws_vpc.tokyo-syslog-vpc.id  
  #cidr_block = var.subnet_cidr_block_aurora_database-private1d
  subnet_ids = [aws_subnet.aurora_database-private-subnet-1d.id, 
                aws_subnet.aurora_database-private-subnet-1c.id, 
                aws_subnet.aurora_database-private-subnet-1a.id]

  tags = {
    Name = "Aurora Database Subnet Group"
  }
}
/////////////////////////////////////////////////////////////////////
#Aurora Database Subnet 
resource "aws_subnet" "aurora_database-private-subnet-1d"{                
    vpc_id = aws_vpc.tokyo-syslog-vpc.id                     
    cidr_block = var.subnet_cidr_block_aurora_database-private1d              
    availability_zone = var.avail_zone_aurora_database-private1d  
    provider    = aws.tokyo     
    tags = {                                           
        Name: "${var.env_prefix_aurora_database}-subnet-1d"  
        Service: "Armageddon Phase 1"   
        Owner = "MATTRESS AVENGERS"         
    }
}

resource "aws_subnet" "aurora_database-private-subnet-1a"{                
    vpc_id = aws_vpc.tokyo-syslog-vpc.id                     
    cidr_block = var.subnet_cidr_block_aurora_database-private1a              
    availability_zone = var.avail_zone_aurora_database-private1a  
    provider    = aws.tokyo     
    tags = {                                           
        Name: "${var.env_prefix_aurora_database}-subnet-1a"  
        Service: "Armageddon Phase 1"   
        Owner = "MATTRESS AVENGERS"         
    }
}

resource "aws_subnet" "aurora_database-private-subnet-1c"{                
    vpc_id = aws_vpc.tokyo-syslog-vpc.id                     
    cidr_block = var.subnet_cidr_block_aurora_database-private1c              
    availability_zone = var.avail_zone_aurora_database-private1c  
    provider    = aws.tokyo     
    tags = {                                           
        Name: "${var.env_prefix_aurora_database}-subnet-1c"  
        Service: "Armageddon Phase 1"   
        Owner = "MATTRESS AVENGERS"         
    }
}