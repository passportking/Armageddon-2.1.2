...
*/
 ///////////////////////////////////////////////////////////////////////////
 # # SYSLOG LOAD BALANCER
 resource "aws_lb" "tokyo_syslog_nlb" {
  name               = "tokyo-syslog-nlb"
  internal          = true
  load_balancer_type = "network"
  subnets           = [aws_subnet.tokyo-syslog-private-subnet-1c.id]
  provider          = aws.tokyo
}

resource "aws_lb_listener" "syslog_tcp" {
  load_balancer_arn = aws_lb.tokyo_syslog_nlb.arn
  port              = "514"
  protocol          = "TCP"
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.tokyo_syslog_tg.arn
  }
}
